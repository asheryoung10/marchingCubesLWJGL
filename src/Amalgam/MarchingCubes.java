...
public class MarchingCubes {
    ...
    public static void init(float[][] cubes, int count) {
        float[][] newCubes = new float[cubes.length][8];

        for(int i = 0; i < cubes.length; i++) {
            newCubes[i][0] = cubes[i][7];
            newCubes[i][1] = cubes[i][3];
            newCubes[i][2] = cubes[i][5];
            newCubes[i][3] = cubes[i][1];
            newCubes[i][4] = cubes[i][6];
            newCubes[i][5] = cubes[i][2];
            newCubes[i][6] = cubes[i][4];
            newCubes[i][7] = cubes[i][0];
        }
        int[] codes = new int[newCubes.length];
        for(int j = 0; j < newCubes.length; j++) {
            int code = 0;
            float[] newCube = newCubes[j];
        ...
        for(int j = 0; j < codes.length; j++) {
            int code = codes[j];
            int[] decoded = TriangleTable[code];
            for(int i = 0; i < decoded.length - 1; i++) {
                triangles.add(edgePositions[decoded[i]*3+0] + 0.5f + x);
                triangles.add(edgePositions[decoded[i]*3+1] + 0.5f + y);
                triangles.add(edgePositions[decoded[i]*3+2] + 0.5f + z);
            }
            z++;
            if(z == count) {
                z = 0;
        ...
        float[] triangleData = new float[triangles.size()];
        for(int i = 0; i < triangles.size(); i++) {
            triangleData[i] = triangles.get(i);
        }
        amount = triangleData.length;

        shader = new Shader("assets/shaders/marchingCubes.glsl");
